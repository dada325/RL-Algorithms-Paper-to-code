...
# Intro-rlax-jax
Let's start with an example: Suppose you want to implement the epsilon-greedy algorithm for a simple reinforcement learning problem. Epsilon-greedy is a simple method to balance exploration and exploitation by choosing between a random action and the action currently believed to be the best with a certain probability.
